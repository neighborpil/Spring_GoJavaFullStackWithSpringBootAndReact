...
import com.neighbopril.rest.webservices.restfulwebservices.jwt.JwtTokenUtil;
...
public class JwtAuthenticationRestController {
    ...
    public ResponseEntity<?> createAuthenticationToken(
        ...
        return ResponseEntity.of(Optional<JwtTokenResponse>new JwtTokenResponse(token));
    }
    ...
    public ResponseEntity<?> refreshAndGetAuthenticationToken(HttpServletRequest request) {
        ...
        JwtUserDetails user = (JwtUserDetails) jwtInMemoryUserDetailsService.loadUserByUsername(
            username);

        if (jwtTokenUtil.canTokenBeRefreshed(token)) {
            String refreshedToken = jwtTokenUtil.refreshToken(token);
            return ResponseEntity.of(new JwtTokenResponse(refreshedToken));
        } else {
